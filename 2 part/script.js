...
var skills = _.map(mas, 'skills');
var result = [];
var buffer = [];
_.forEach(skills, function(o){
  var ff = _.difference(o, buffer);
  _.forEach(ff, function(d){
    result.push(d);
  })
  _.forEach(o, function(g){
    buffer.push(g);
  })
});
skills = null;
...
//Names
var names = _.map(mas, 'name');
var finalNames = [];
_.forEach(names, function(name){
  var counter = 0;
  _.forEach(mas, function(obj){
    if(counter == 0) {
      finalNames.push(name);
    };
    counter++;
    if (obj.name == name) {
      finalNames.push(obj.friends);
    }
  })
})
...
for (var i = 0; i < finalNames.length; i++) {
  if (!(i%2 == 0)) {
    man.push(finalNames[i].length);
    mans.push(man);
    man = [];
  } else {
    man.push(finalNames[i]);
  }
}
mans = _.sortBy(mans, function(d){
  return d[1];
});
